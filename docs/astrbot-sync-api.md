...
> 版本 v1 · 对应服务端 `src/` 实现。积分真源在插件侧（AstrBot 积分插件），本系统只发「指令」，插件执行入账并回报。
>
...
- **插件 → 服务器**：绑定码认领 + 待发内容拉取（战报、开放通知、截止提醒）。插件定时轮询，不依赖服务器入站可达。
...
### 3.2 GET /api/reports/pending —— 拉取待发内容

```json
{ "reports": [
  { "id": "rp-<uuid>", "event_id": 1, "kind": "report", "content": "🏆 竞猜战报 · 英超第3轮\n...", "created_at": "..." },
  { "id": "nt-<uuid>", "event_id": 2, "kind": "open",   "content": "🎯 新竞猜开放《英超第3轮》\n...", "created_at": "..." },
  { "id": "nt-<uuid>", "event_id": 2, "kind": "remind", "content": "⏰ 《英超第3轮》还有约 4 小时截止\n...", "created_at": "..." }
] }
```

最多 5 条/次，按时间正序。`content` 都是可直接发群的纯文本，插件**不需要按 `kind` 分支处理**，原文发到目标群即可。

| kind | 何时入队 | 内容 |
|---|---|---|
| `report` | 确认发奖后 | 竞猜战报（命中明细 + 总发放积分），默认值 |
| `open` | 竞猜开放时（建期勾「立即开放」或管理台点「开放」） | 新竞猜开放 + 场次数 + 截止时间 + 最高可得积分 + 站点链接 |
| `remind` | 截止前 4 小时内（每场竞猜最多一条） | 剩余时间 + 已有多少人提交 + 站点链接 |

`kind` 只在你需要区分发送目标（比如通知不发群、只发管理员）时才有用。
